...
city_gdf = gpd.read_file('City_Boundary.geojson')  # Fixed file name
...
# Store results: {day: {hour: (top_origins, top_destinations)}}
        ...
        with open(filename, 'r') as f:
            ...
            for row in reader:
                try:
                    ...
                    counts[dest] = counts.get(dest, 0) + count

                    All_top[hour][origin] = All_top[hour].get(origin, 0) + count
                    All_top[hour][dest] = All_top[hour].get(dest, 0) + count
                except Exception as e:
                    continue
        # Store as list of (region, count) tuples, sorted by count
        top_regions = heapq.nlargest(NUM_TOP, counts.items(), key=lambda x: x[1])
        top_per_file[day][hour] = top_regions
...
def plot_highlight(hour):
    ax.cla()
    top_regions = top_per_file[current_day][hour]
    ...
    ax.set_title(f"Top {NUM_TOP} Regions for {pretty_day[current_day]} Hour {hour:02d}:00", fontsize=15)
    ax.set_axis_off()
